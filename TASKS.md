...
*Для решения этой задачи необходимо скачать на сайте файл с входными данными. Ваш файл может отличаться. Папка с решением задачи Task07 содержит один из вариантов входного файла __108.data__.*
...
*Для решения этой задачи необходимо скачать на сайте файл с входными данными. Ваш файл может отличаться. Папка с решением задачи Task08 содержит один из вариантов входного файла __40.data__.*
...
*Во входном файле __46.txt__ ни один из судей не совершил ошибку. Тем не менее в качестве правильного ответа засчитывается __-97 49266 pluto__.*

*Для решения этой задачи необходимо скачать на сайте файл с входными данными. Ваш файл может отличаться. Папка с решением задачи Task09 содержит два варианта входного файла __46.txt__ и __48.txt__.*
...
*Для решения этой задачи необходимо скачать на сайте файл с входными данными. Ваш файл может отличаться. Папка с решением задачи Task10 содержит один из вариантов входного файла __50.data__.*
...
*Для решения этой задачи необходимо скачать на сайте файл с входными данными. Ваш файл может отличаться. Папка с решением задачи Task11 содержит один из вариантов входного файла __70.data__.*
...
*Для решения этой задачи необходимо скачать на сайте файл с входными данными. Ваш файл может отличаться. Папка с решением задачи Task12 содержит один из вариантов входного файла __52.data__.*
...
*Для решения этой задачи необходимо скачать на сайте файл с входными данными. Ваш файл может отличаться. Папка с решением задачи Task14 содержит один из вариантов входного файла __78.txt__.*
...
*Для решения этой задачи необходимо скачать на сайте файл с входными данными. Ваш файл может отличаться. Папка с решением задачи Task15 содержит один из вариантов входного файла __81.txt__.*
...
*Для входного файла __87.txt__ правильным ответом является число __49__. Тем не менее в качестве правильного ответа засчитывается число __56__.*

*Для решения этой задачи необходимо скачать на сайте файл с входными данными. Ваш файл может отличаться. Папка с решением задачи Task16 содержит один из вариантов входного файла __87.txt__.*
...
Необходимо вывести 0, если ошибкок не было.
Если ошибки были — необходимо вывести номера строк (номера начинаются с 1, выведите их через пробел), соответствующей песни.

*Ошибкой считается попадание в свет софита.*

*Для решения этой задачи необходимо скачать на сайте файл с входными данными. Ваш файл может отличаться. Папка с решением задачи Task17 содержит один из вариантов входного файла __89.txt__.*


### Задача 18
Чтобы жюри международного конкурса исполнителей «Еврослух» больше не обвиняли в необъективном судействе, организаторы конкурса решили внедрить им в помощь автоматическую систему оценки исполнителей. Эта система автоматически фиксирует правильность исполнения танца по координатам исходя из попадания в свет софита и подсчет штрафных баллов за ошибки.
Прошлый опыт с треугольными софитами показал себя не очень эффектным, поэтому светорежиссер поставил обычные, круглые софиты. При этом если участник конкурса попадает на границу освещения, то система не фиксирует ошибку.
...
Программа должна посчитать количество штрафных баллов, а также указать в какие круги-диапазоны попал исполнитель (указать строчки во входном файле). Для расчётов считать размер штрафа за попадание в свет софита с ошибкой — 499 баллов, сначала выводить все номера диапазонов (номера начинаются с 1), потом — сумму. Если исполнитель не попал ни в один круг, нужно вывести 0.

*Ошибкой считается попадание в свет софита.*

*Для решения этой задачи необходимо скачать на сайте файл с входными данными. Ваш файл может отличаться. Папка с решением задачи Task18 содержит один из вариантов входного файла __94.txt__.*


### Задача 19
Перед последним отборочным этапом конкурса жюри нужно узнать имя того, кто чаще других фальшивил. Для этого базу, представляющую собой список проштрафившихся певцов, передали тебе: ведь ты не только отличный музыкант, но еще и программист.
На входе программы — список с именами плохих исполнителей в хронологическом порядке, одна строчка, одно имя (за основу взяты славянские имена из соответствующей статьи Википедии).
Если горе-певцов несколько, то нужно вывести всех в алфавитном порядке через запятую.

*Для решения этой задачи необходимо скачать на сайте файл с входными данными. Ваш файл может отличаться. Папка с решением задачи Task19 содержит один из вариантов входного файла __97.txt__.*


### Задача 20
Сенсационное открытие сделали недавно британские астрофизики: на других планетах не только есть жизнь, но и своя поп-культура! Анализируя полученные сообщения с радиотелескопа (одно из них дается вам в качестве задания) они обнаружили фрагменты ритуальных песен инопланетян!
Установлено, что в песнях очень часто повторяются похожие пары слов (например, «hey сortana» или “ok google” – будем называть их значимыми парами), перемежаемые большим количеством слов, не поддающихся расшифровке. Для общения с инопланетянами предлагается послать им обратно послание, собранное из значимых пар следующим образом: берем в порядке убывания частоты все первые слова пар, и для каждого такого слова в качестве второго берем самое часто встречающееся из вторых слов для данного первого слова. Полученные слова надо записать подряд в виде единого текста для передачи на радиотелескоп.
Вам предлагается написать такую программу для генерации ответного послания. Удачи!

*Для решения этой задачи необходимо скачать на сайте файл с входными данными. Ваш файл может отличаться. Папка с решением задачи Task20 содержит один из вариантов входного файла __100.data__.*
